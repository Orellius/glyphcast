...
const SPECS: PaneSpec[] = [
  ...
  { key: 'wire565', label: 'wire RGB565 (bandwidth tier)', mode: 'octant', kind: 'wire', depth: '565' },
  { key: 'wire888', label: 'wire RGB888 (fidelity tier)', mode: 'octant', kind: 'wire', depth: '888' },
]
  ...
  if (video.readyState >= 2) {
    for (const p of panes) {
      if (p.kind === 'direct') {
        ...
        // honest wire pane: what the receiver actually sees after the real
        // pack -> unpack roundtrip at this depth
        const img = p.sampler.sample(video, p.cols * sampleX(p.mode), p.rows * sampleY(p.mode))
        encodeCells(img, p.cols, p.rows, p.mode, 0, p.fg, p.bg)
